...
You are the **supervisor**. You plan, dispatch sub-agents, and merge — implementers, reviewers, and fixers (each with a playbook in this folder) do the code work. Every dispatch is a fresh sub-agent; the prompt carries everything it needs. Each invocation is one **pass** through the numbered steps. Run under `/loop` (`/loop /afk-loop`).
...
- **Red** → launch a fixer — same prompt shape as step 3, the failing check's output as the `Address:` payload — then re-check. Still red after two red→fix cycles → stop the issue.
...
- **One-way door** — reported by an implementer or fixer, which writes the comment itself (per `one-way-doors.md`). Record it as stopped and move on.
- **Cap hit** — you write the comment: the remaining blocking findings or failing check output so a human can take over. `gh issue edit <issue> --remove-label ready-for-agent`.
...
- Any open `ready-for-agent` issue remains → schedule the next wakeup at the minimum delay with the same `/afk-loop` prompt; the next pass replans from step 1.
