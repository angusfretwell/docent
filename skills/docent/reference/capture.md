...
```bash
npx -y docent walkthrough create --kind product
...
npx -y docent capture add --walkthrough wlk_… --kind screenshot --media <tmp>.rrweb.json \
  --route /signup --viewport 1280x800 --dims 1280x2400 --title "Empty signup form"

# recording: --duration-ms rides instead of --dims
npx -y docent capture add --walkthrough wlk_… --kind recording --media <tmp>.rrweb.json \
