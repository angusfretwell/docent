...
```bash
npx -y @angusfretwell/docent@latest comment list                              # the whole queue
npx -y @angusfretwell/docent@latest comment list --status open                # only comments someone owes work on
npx -y @angusfretwell/docent@latest comment list --status open,actioned       # everything unresolved (any-of: comma or repeat)
npx -y @angusfretwell/docent@latest comment list --status actioned            # handed back — awaiting verification
npx -y @angusfretwell/docent@latest comment list --anchor-file src/app.ts     # anchored on this file
npx -y @angusfretwell/docent@latest comment list --author claude-code         # this author participated
```
...
npx -y @angusfretwell/docent@latest comment add --file src/app.ts --line 42:47 --body "This early-return leaks the lock."
npx -y @angusfretwell/docent@latest comment add --file src/app.ts --body "This module has no exports."            # whole file, default side head
npx -y @angusfretwell/docent@latest comment add --file src/app.ts --line 10 --side base --body "This was the safe version."
npx -y @angusfretwell/docent@latest comment add --change <<'EOF'                                                  # long body via stdin — omit --body
...
```bash
npx -y @angusfretwell/docent@latest comment add --change --body "…" --agent claude-code --model claude-fable-5
npx -y @angusfretwell/docent@latest comment action --comment cmt_… --agent claude-code
```
...
1. **Fetch the work — default to the open queue.** With no filter from the human, pull `--status open` — the fixer's inbox, where fresh Comments, plain comments, and "do it again" re-comments all fold. Reach past it with the filters above when the human deliberately wants another slice (verifying actioned Comments, reviewing resolved ones, one file's worklist).
   ...
   ```bash
   npx -y @angusfretwell/docent@latest comment list --status open
   ```
...
1. **Take stock of the session — record only what happened.** Never invent a Comment the session didn't raise, a hand-back for work it didn't do, or a resolution it didn't verify. Sort what actually happened:
   ...
   | --- | --- | --- |
   | Raised a new review concern | a **fresh Comment**, born open | `npx -y @angusfretwell/docent@latest comment add` |
   | Addressed a Comment pulled via `--read` | a **reply**, then a **hand-back** | `npx -y @angusfretwell/docent@latest comment reply` + `npx -y @angusfretwell/docent@latest comment action` |
   | Looked at a claimed fix and found it wrong | a **reply** alone (leaves it open) | `npx -y @angusfretwell/docent@latest comment reply` |
   | Verified a fix that now holds | a **resolve** | `npx -y @angusfretwell/docent@latest comment resolve` |

   Then fetch the queue (`npx -y @angusfretwell/docent@latest comment list --status open,actioned`) so a reply or resolve lands on the Comment it belongs to and you don't re-raise something already open. Work done against a Comment already in the queue is a **reply**; a genuinely new concern with no open Comment is a fresh **add**. When in doubt, match the concern's anchor against the queue.
...
5. **Confirm.** Re-list the queue (`npx -y @angusfretwell/docent@latest comment list --status open,actioned`) to confirm the outcomes landed: fresh Comments present, turns handed back, verified fixes closed.
