...
The session-side companion to the `docent` tool — a docent gives the guided tour. It assumes only a git repository and the tool's `.docent/` state directory at the repo root (auto-created on first write). The `docent` CLI is reached through `npx docent`, which self-bootstraps its per-platform binary on first run — no global install needed.
...
```bash
npx -y docent --version
```

On a non-zero exit the CLI could not bootstrap. Relay the command's own stderr — it is the authority on why (`unsupported platform: …`, `download failed (NNN): …`, or Node/npx missing) — wrapped in an actionable line, e.g. `` `npx docent` couldn't run — <stderr>; ensure Node ≥18 and network access, then re-run /docent ``, and **hard-stop**. Nothing runs on a failed gate. The check is stateless — it runs every invocation, because machine capability can regress (evicted cache, Node change) and a cached "passed" would skip a check that should now fail — and cheap once the binary is cached; the `-y` also warms that cache up front, where the human is watching, rather than mid-capture.
...
- **Name the change.** Reading the head is also where you learn what this branch _is_, so record it as the Review's title — the headline the UI renders:
  ...
  ```bash
  npx -y docent review set --title "Palette panel"
  ```
...
```bash
npx -y docent status          # → { "serving": true, "url": "http://127.0.0.1:…/" }  or  { "serving": false }
```
...
- **Not serving** → start one in the background (it runs until the human stops it), poll until it answers, then open the browser. **Bound the poll** — on timeout hard stop with an actionable message:
  ...
  ```bash
  npx -y docent serve >/dev/null 2>&1 &   # backgrounded; leave it running
  for _ in $(seq 50); do           # `docent serve` records its address on boot; poll it, bounded (~10s)
    npx -y docent status | grep -q '"serving": true' && break
    sleep 0.2
  done
  npx -y docent status | grep -q '"serving": true' || {
    echo "docent serve did not come up within ~10s — run 'npx -y docent serve' in this repo to see the boot error, then re-run /docent" >&2
    exit 1
  }
  ```
