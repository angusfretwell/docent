...
| Written against an earlier commit | "The code walkthrough was written 3 commits back — writing a fresh one." |
| Written against this head | "The product walkthrough is up to date — leaving it." |

Count those commits rather than making the human infer the gap, and say nothing about a first run being missing or empty — a branch with no walkthrough yet is simply a clean start:

```bash
git rev-list --count <that walkthrough's headSha>..HEAD
```
