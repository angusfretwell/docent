 ...
 * merged in, so the serve boot and `index.test.ts` share one wiring.
 */
...
/**
 * Build the `request → Promise<Response>` handler the serve boot (`./serve`)
 * hands to `Bun.serve`'s `fetch` and the serve tests call directly. Effect
 * lives one level down here (the serve boot is plain Bun) because Effect's Bun
 * server swaps handlers via `server.reload`, which wipes Bun's HTML-bundle
 * `routes` table (@see docs/adr/0001-serve-via-bun-fullstack.md).
 *
