...
import { Anchor, Author } from "@shared/schemas/comment";
import { CommentWrite } from "@shared/schemas/comment-write";
import type { CommentId } from "@shared/schemas/ids";
...
const commitWrite = Effect.fn("commitWrite")(function* commitWrite(
  scope: WriteScope,
  author: Author,
  draft: typeof CommentWrite.Encoded
) {
  const write = yield* Schema.decodeUnknownEffect(CommentWrite)(draft).pipe(
    Effect.mapError(
      (error) =>
        new CliUsageError({ reason: `invalid comment write: ${error}` })
    )
  );
  return yield* writeCommentRecord({
    author,
    base: scope.base,
    branch: scope.branch,
    refs: scope.refs,
...
export const addComment = Effect.fn("addComment")(function* addComment(
  cwd: string,
...
export const replyComment = Effect.fn("replyComment")(function* replyComment(
  cwd: string,
  params: { author: AuthorOpts; body: string; commentId: CommentId }
) {
  const scope = yield* resolveChangeScope(cwd);
  const author = yield* buildAuthor(scope.root, params.author);
  return yield* commitWrite(scope, author, {
...
export const actionComment = Effect.fn("actionComment")(function* actionComment(
  cwd: string,
  params: { author: AuthorOpts; commentId: CommentId }
) {
  const scope = yield* resolveChangeScope(cwd);
  const author = yield* buildAuthor(scope.root, params.author);
  return yield* commitWrite(scope, author, {
...
export const resolveComment = Effect.fn("resolveComment")(
...
export const reopenComment = Effect.fn("reopenComment")(function* reopenComment(
  cwd: string,
  params: { author: AuthorOpts; commentId: CommentId }
) {
  const scope = yield* resolveChangeScope(cwd);
