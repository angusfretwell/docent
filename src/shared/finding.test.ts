...
import type { FindingRecord } from "./finding.ts";
...
describe("foldFinding", () => {
  ...
  test("collects replies and unique participants in order", () => {
    const folded = foldFinding("fnd_1", [
      record({ anchor: lineAnchor, author: claude, name: "001-open.md", type: "open" }),
      record({
        author: angus,
        body: "fixed",
      ...
      record({ author: angus, body: "and again", name: "003-reply.md", type: "reply" }),
    ]);

    expect(folded.replies.map((reply) => reply.body)).toEqual(["fixed", "and again"]);
  ...
  test("a resolve record closes the finding", () => {
    const folded = foldFinding("fnd_1", [
      record({ anchor: lineAnchor, name: "001-open.md", type: "open" }),
      record({ body: "verified under load", name: "002-resolve.md", type: "resolve" }),
    ]);
  ...
  test("an edit record supersedes the named record's body", () => {
    const folded = foldFinding("fnd_1", [
      record({ anchor: lineAnchor, body: "original open body", name: "001-open.md", type: "open" }),
      ...
      record({ body: "corrected reply", edits: "002-reply.md", name: "004-edit.md", type: "edit" }),
    ]);

    expect(folded.body).toBe("corrected open body");
    expect(folded.replies.map((reply) => reply.body)).toEqual(["corrected reply"]);
  });
...
describe("findingLocation", () => {
  ...
  test("renders a file anchor as the path", () => {
    expect(
      findingLocation({ blobSha: "abc", file: "src/main.ts", kind: "file", side: "head" }),
    ).toBe("src/main.ts");
  });
