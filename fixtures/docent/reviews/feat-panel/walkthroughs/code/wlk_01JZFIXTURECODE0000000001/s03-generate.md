...
`generate` collects five random colors and hands them to `render`, then runs once on load so the page is never empty. The Generate button wires the same handler.
