...
- **The filesystem is the interface** ([#2](https://github.com/angusfretwell/docent/issues/2)). All review state persists as plain, self-describing files under `.docent/`. Agents read and write those files directly; the `docent` binary is a renderer plus optional validating sugar over the very same files — never a gate. That persistence is **machine-local**: the `.docent/.gitignore` commit policy keeps Findings, Changes, and Walkthroughs as working state that does not survive a clone — only the capture runbook and the policy travel with the repo (data-model.md §1, [#78](https://github.com/angusfretwell/docent/issues/78)).
...
- **Installing the skills** → the **tool**. `docent install` is the onboarding wizard: it installs this catalogue via the skills CLI and seeds the `.docent/.gitignore` commit policy — it is a subcommand, not itself a skill. Distribution and install decisions are owned by [architecture.md §5](architecture.md) ([#78](https://github.com/angusfretwell/docent/issues/78)).
