...
- **Minted lazily on first reference** ([#24](https://github.com/angusfretwell/docent/issues/24)). The diff itself always renders the live head straight from git; a Change crystallizes only when a durable artifact must reference the head — a Finding record (which stamps `changeId`, [#20](https://github.com/angusfretwell/docent/issues/20)), a Walkthrough (`bornChangeId`), or a `/to-docent` write referencing the head. Minting is **idempotent by identity**: the same `(baseSha, headSha)` never mints twice.
...
This backpressure handling drops chunks when the consumer stalls — the `highWaterMark` check on line 44 races the flush.
```
...
```json
{
  "path": "src/parser/stream.ts",
  "blobSha": "9c2a1f0…",
  "ts": "2026-07-10T02:31:07Z"
}
```
