...
- **Local-branch-centric.** v1's sole input is a local git branch checked out in the repo. There is no GitHub integration — no PR input, no API reads. (A PR returns later as additive *provenance* on a Change, never identity.)
...
| [architecture.md](architecture.md) | The app shell: Bun-native local server, React+Vite UI, HTTP file API + SSE live-reload, packaging and delivery (`npx docent`). |
