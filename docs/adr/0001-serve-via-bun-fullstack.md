...
Serving details are specified in [architecture.md](../spec/architecture.md) §§1, 4, 5; the web handler and its wiring live in `src/api/` (`index.ts`), the serve boot in `src/serve.ts`, and the entry point is `src/docent.ts`.
