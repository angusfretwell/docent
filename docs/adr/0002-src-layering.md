...
- **Surfaces stay thin.** cli and api validate at the boundary with schemas defined in `shared` and delegate to core. The one-write-implementation invariant for comment records holds: both surfaces call the same core write. Operation logic found living in a surface moves down — e.g. `buildAnchor` (resolving a code anchor's content-addressed `blobSha` from git) moved from the comment CLI to `core/git/anchor.ts`.
