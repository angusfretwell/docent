...
- **Dev is Bun's HTML dev server.** `bun run dev:site` serves the HTML entries with hot reload; the bunfig `[serve.static]` plugin registration is what compiles Tailwind there. Once the site grew its second page it stopped being a bare `bun src/site/index.html`: the demo is an SPA, so `scripts/dev-site.ts` routes `/demo/*` to its entry and serves the two files the build copies rather than bundles (the diff worker and the captured snapshot) at the paths the demo fetches them from.
